...
class ipb3_007 extends ipb3_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($data_array['data'] as $post_id => $post_details)
		{
			...
			$pagetext = $this->snapback($Db_target, $target_database_type, $target_table_prefix, $text);
	...
	function snapback(&$Db_target, &$target_database_type, &$target_table_prefix, $text)
	{
		if (strpos($text, '[snapback]') !== false)
		{
			...
			preg_match_all('#\[snapback\](.*)([0-9]+)\[/snapback\]#siU', $text, $matches);
			if ($matches)
			{
				for($x = 0; $x < count($matches); $x++)
				{
					if ($matches[3][$x])
					{
						$new_id = $this->get_vb_post_id($Db_target, $target_database_type, $target_table_prefix, $matches[3][$x]);
						$text = str_replace($matches[0][$x], "[post={$new_id}]Quoted post[/post]", $text);
					}
					else
					{
						return $text;
		...
		{
			preg_match_all('#\[quote name=\'(.*)\' (?:date|timestamp)=\'(.*)\' post=\'(\d+)\'(.*)\](.*)\[/quote\]#siU', $text, $matches);
			if ($matches)
			{
				for($x = 0; $x < count($matches); $x++)
				{
					if ($matches[3][$x])
					{
						...
						if ($new_id)
						{
							$text = str_replace($matches[0][$x], "[QUOTE=" . $matches[1][$x] . ";{$new_id}]" . trim($matches[5][$x]) . "[/QUOTE]", $text);
						}
						else
						{
							$text = str_replace($matches[0][$x], "[QUOTE=" . $matches[1][$x] . "]" . trim($matches[5][$x]) . "[/QUOTE]", $text);
						}
					}
				}
			}
		...
		$text = preg_replace('#\[quote name=\'(.*)\'\](.*)\[/quote]#siU', '[quote=$1]$2[/quote]', $text);
