...
class vbcms_003 extends vbcms_000
{
	var $_dependent = '002';

	function vbcms_003(&$displayobject)
	{
		$this->_modulestring = $displayobject->phrases['import_cms_layout'];
	}
	...
	function init(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($this->check_order($sessionobject,$this->_dependent))
		{
			if ($this->_restart)
			{
				if ($this->restart($sessionobject, $displayobject, $Db_target, $Db_source, 'clear_imported_cms_layouts'))
				{;
					$displayobject->display_now("<h4>{$displayobject->phrases['cms_layouts_cleared']}</h4>");
					$this->_restart = true;
				}
				else
				{
					$sessionobject->add_error($Db_target, 'fatal', $class_num, 0, $displayobject->phrases['cms_layout_restart_failed'], $displayobject->phrases['check_db_permissions']);
				}
			}

			// Start up the table
			$displayobject->update_basic('title', $displayobject->phrases['import_cms_layout']);
