...
class joomla_cms_004 extends joomla_cms_000
{
    ...
    function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
    {
        ...
        foreach ($article_array['data'] as $article_id => $data)
        {
            ...
            $article->set_value('mandatory', 'pagetext',       ($data['fulltext'] != NULL ? $data['fulltext'] : $data['introtext']));

            $article->set_value('nonmandatory', 'title',            $data['title']);
			$article->set_value('nonmandatory', 'poststarter',   $idcache->get_id('user', $data['created_by']));
            $article->set_value('nonmandatory', 'postauthor',    $idcache->get_id('username', $data['created_by']));
            $article->set_value('nonmandatory', 'previewtext',   substr($data['introtext'], 0, 2040));
            ...
            if($article->is_valid())
            {
                ...
                if($contentid = $article->import_cms_article($Db_target, $target_database_type, $target_table_prefix))
                {
					...
					$node->set_value('nonmandatory', 'userid', $idcache->get_id('user', $data['created_by']));
